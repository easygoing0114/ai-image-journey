 ...
 * and initialize them as Bluesky embeds. Also, extract the first 50 characters
 * from the first <p> element to create a <title> element.
 *
...
function scan(node) {
    ...
    for (var i = 0; i < embeds.length; i++) {
        ...
        // Extract first 50 characters from the first <p> element
        var firstParagraph = node.querySelector('p');
        if (firstParagraph) {
            var titleText = firstParagraph.textContent.slice(0, 50);
            if (titleText.length > 0) {
                iframe.title = titleText;
            } else {
                iframe.title = 'Bluesky Embed';
            }
        } else {
            iframe.title = 'Bluesky Embed';
        }
